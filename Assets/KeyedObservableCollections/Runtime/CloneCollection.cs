...
namespace Hg.KeyedObservableCollections.Internal
{
    /// <summary>
    ...
    internal struct CloneCollection<T> : IDisposable
    {
        ...
        public CloneCollection(IEnumerable<T> source)
        {
            ...
            {
                var tempArray = ArrayPool<T>.Shared.Rent(count);

                if (source is ICollection<T> c)
                {
                    c.CopyTo(tempArray, 0);
                }
                else
                {
                    var i = 0;
            ...
            else
            {
                ...
                foreach (var item in source)
                {
                    TryEnsureCapacity(ref tempArray, i);
                    tempArray[i++] = item;
                }
                this.array = tempArray;
                this.length = i;
        ...
        {
            var tempArray = ArrayPool<T>.Shared.Rent(source.Length);
            source.CopyTo(tempArray);
            this.array = tempArray;
